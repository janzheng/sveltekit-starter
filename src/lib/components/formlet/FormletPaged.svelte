        ...
        <div class="Formlet-page-container __page-{curPageNumber}" transition:fade="{{duration: 350}}">
          ...
          <div class="Formlet-page-pagination _align-vertically {curPageNumber==0?'':'grid grid-cols-1-2'}">
            ...
            {#if formData.settings && formData.settings.submitText && curPageNumber == formPages.length-1}
              {#if isSubmitting}
                <button type="submit" class="submittingButtonClasses Btn-solid __cta ease-in-out mb-0 {formStyles['submittingButtonClasses'] || ''} ">{formData.settings&&formData.settings.submittingText || "Submitting..."}</button>
              {:else}
                <button type="submit" class="submitButtonClasses Btn-solid ease-in-out mb-0 {formStyles['submitButtonClasses'] || ''} ">{formData.settings&&formData.settings.submitText || "Submit"}</button>
              {/if}
            {/if}
